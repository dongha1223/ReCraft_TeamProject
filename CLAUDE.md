...
- **⚠ 런타임 스프라이트/텍스처 생성 주의** (`new Texture2D`, `Sprite.Create` 등 런타임 에셋 생성은 
Inspector 관리 불가·최적화 저해 우려가 있으므로 사용 전 반드시 사용자에게 경고할 것. 스프라이트가 필요한 경우 에셋을 직접 준비하는 방식 권장)
- 최적화 요청시 성능 최적화와 코드 품질 개선(너무 긴 줄이나 반복구문)도 함께 진행. 
